...
export default function Home(): JSX.Element {
  ...
  const { context } = state;
  const { data, isError, isNotPlaying, artistImg } = useSpotifyData(
    context.accessToken
  );
  const { data: tokens } = useSWR(
    context.refreshToken ? `/api/refresh_token/${context.refreshToken}` : null,
  ...
  return (
    ...
    >
      {!fullscreen.active && (
        <Topbar>
          <IconButton
            onClick={() => send({ type: "SWITCH_PALETTE" })}
            icon={Palette}
          />
          <IconButton
            onClick={() => {
